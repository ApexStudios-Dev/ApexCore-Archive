...
final class RegistrarImpl<T> implements Registrar<T>
{
    ...
    public Registry<T> getVanillaRegistry()
    {
        ...
        return RegistryHooks.findVanillaRegistry(registryType).orElseThrow();
    }
    ...
    public void register()
    {
        ...
        ApexCore.LOGGER.debug(RegistrarManager.MARKER, "[{}] Registering registry entries of type '{}'", getOwnerId(), registryType.location());
        RegistryHooks.get().register(this);
        registered = true;
    }
    ...
    public RegistryEntry<T> registerOptional(ResourceLocation registryName)
    {
        ...
        ApexCore.LOGGER.debug(RegistrarManager.MARKER, "[{}] Captured registration of optional registry entry '{}' for type '{}'", getOwnerId(), registryType.location(), registryType.location());
    ...
    {
        RegistryHooks.get().addListener(registryType, getOwnerId(), registryName, listener);
    }
    // endregion
    ...
    public <R extends T> RegistryEntry<R> register(String registrationName, Supplier<R> registryEntryFactory)
    {
        ...
        ApexCore.LOGGER.debug(RegistrarManager.MARKER, "[{}] Captured registration of registry entry '{}' for type '{}'", getOwnerId(), registryName, registryType.location());
        var registryEntry = RegistryHooks.get().register(this, registrationName, registryEntryFactory);
